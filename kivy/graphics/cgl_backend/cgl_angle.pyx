...
cdef extern from *:
    ...
    #if !__USE_ANGLE_GL_BACKEND
        static void* eglGetProcAddress(const char* name) { return 0; }
    #endif
    """
...
def init_backend():
    IF not USE_ANGLE_GL_BACKEND:
        raise TypeError('ANGLE GL backend is not available. Recompile with USE_ANGLE_GL_BACKEND=1')
    ELSE:
        # EGL context must have been created by now (by EGLANGLE)
